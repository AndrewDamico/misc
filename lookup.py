...
# # Lambda Function We can use a lambda function to iterate through the rows, finding the location of the value x in
# columns B, and returning the subsequent value in column A.

df['C'] = df['A'].apply(lambda x: df['A'].loc[df['B'] == x].values[0])
print(df)

# # Iterate through rows The following performs the same operations, but in a non-lambda format (i.e., a for loop).
# This will find the first location of the value in column A in column B, and will then return the value in column A
# corresponding to the row in column B
